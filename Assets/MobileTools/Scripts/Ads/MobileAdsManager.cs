...
namespace BBG.MobileTools
{
	public class MobileAdsManager : SingletonComponent<MobileAdsManager>
	{
		...
		public System.Action OnPaidEvent					{ get; set; }
		...
		private void CreateAdNetworkHandlers()
		{
			...
			if (AreBannerAdsEnabled)
			{
				try
				{
					...
					BannerAdHandler.OnPaidEvent				+= BannerPaidEvent;
				}
			...
			if (AreInterstitialAdsEnabled)
			{
				try
				{
					...
					InterstitialAdHandler.OnPaidEvent					+= InterstitialPaidEvent;
				}
				catch(System.Exception ex)
				{
			...
			if (AreRewardAdsEnabled)
			{
				try
				{
					...
					RewardAdHandler.OnPaidEvent				+= RewardAdPaidEvent;
				}
				catch(System.Exception ex)
				{
		...
		private void BannerPaidEvent(AdValue args, string adUnit)
		{
#if BBG_ADMOB
			//if(OnPaidEvent != null)
			//{
			...
			print(MobileAdsSettings.Instance.adMobConfig.BannerPlacementId + " : " + adUnit);

			if(!adUnit.Equals(MobileAdsSettings.Instance.adMobConfig.BannerPlacementId)) return;

				AdValue paidAdValue = args;

				var revenue = new YandexAppMetricaRevenue((decimal)(paidAdValue.Value / 1000000f), paidAdValue.CurrencyCode);

				revenue.ProductID = MobileAdsSettings.Instance.adMobConfig.BannerPlacementId;
				revenue.Receipt = new YandexAppMetricaReceipt();

				AppMetrica.Instance.ReportRevenue(revenue);
				FirebaseManager.ReportRevenue(revenue.ProductID, paidAdValue.Value / 1000000f, paidAdValue.CurrencyCode);

				Debug.Log($"AdMob Report revenue AdUnit: {revenue.ProductID} Value: {paidAdValue.Value / 1000000f} Currency: {paidAdValue.CurrencyCode}");
			//}
#endif
		}
		...
		private void InterstitialAdClosed()
		{
			...
			AnalyticEvents.ReportEvent("interstitial_exit");
		}

		private void InterstitialPaidEvent(AdValue args, string adUnit)
		{
#if BBG_ADMOB
			//if(OnPaidEvent != null)
			//{
			...
			print(MobileAdsSettings.Instance.adMobConfig.InterstitialPlacementId + " : " + adUnit);

			if(!adUnit.Equals(MobileAdsSettings.Instance.adMobConfig.InterstitialPlacementId)) return;

			AdValue paidAdValue = args;

				var revenue = new YandexAppMetricaRevenue((decimal)(paidAdValue.Value / 1000000f), paidAdValue.CurrencyCode);

				revenue.ProductID = MobileAdsSettings.Instance.adMobConfig.InterstitialPlacementId;
				revenue.Receipt = new YandexAppMetricaReceipt();
		...
		private void InterstitialAdShowing()
		{
			...
			AnalyticEvents.ReportEvent("interstitial_start");
		...
		private void RewardAdPaidEvent(AdValue args, string adUnit)
		{
#if BBG_ADMOB
			//if(OnPaidEvent != null)
			//{
			...
			print(MobileAdsSettings.Instance.adMobConfig.RewardPlacementId + " : " + adUnit);

			if(!adUnit.Equals(MobileAdsSettings.Instance.adMobConfig.RewardPlacementId)) return;

			AdValue paidAdValue = args;

				var revenue = new YandexAppMetricaRevenue((decimal)(paidAdValue.Value / 1000000f), paidAdValue.CurrencyCode);

				revenue.ProductID = MobileAdsSettings.Instance.adMobConfig.RewardPlacementId;
				revenue.Receipt = new YandexAppMetricaReceipt();
